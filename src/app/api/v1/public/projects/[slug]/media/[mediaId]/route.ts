...
import { localMediaPath } from "@/lib/storage/media-storage";
...
// Everything else is 404, so nothing about an asset is revealed. The local
// adapter streams the file, with byte ranges for video; a provider adapter
// would redirect to a short-lived signed URL instead.
export const GET = publicRoute(
  async ({ request, params: { slug, mediaId }, db }) => {
    ...
    if (!asset?.storageKey || asset.storageProvider !== "local") throw notFound();

    const path = localMediaPath(asset.storageKey);
